...
class RsvpController extends ApplicationController {
	...
	function addGuest() {
		...
		if( !is_object( $existing_g ) ) {
			...
			if( strlen( $_REQUEST['first_name'] ) == 0 ) { $errors[] = 'Please specify a first name.'; }
			if( strlen( $_REQUEST['last_name'] ) == 0 ) { $errors[] = 'Please specify a last name.'; }
	...
	function removeGuest() {
		...
		if( $g ) {
			...
			$g = Guest::retrieveByPk( $_REQUEST['guest_id'] );
			echo "active again: " . $g->getActive();exit;
		}
		die();
	}//removeGuest()
